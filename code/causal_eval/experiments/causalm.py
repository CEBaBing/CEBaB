...
from methods.utils.constants import WANDB_PROJECT, CAUSALM_TASK_LABEL_COL, LABEL_COL
from methods.utils.data_utils import get_cebab, preprocess_cebab_for_causalm
from methods.utils.methods_utils import task_to_keys
...
class ModelArguments:
    ...
    task_type: str = field(
        ...
        metadata={"help": f"The type of head for the task. Currently supports only '{SEQUENCE_CLASSIFICATION}'."}
    )
    tc_heads_types: List[str] = field(
        default=None,
        metadata={"help": f"A list of head types for the treated concept. Currently supports only"
                          f" '{SEQUENCE_CLASSIFICATION}' and '{TOKEN_CLASSIFICATION}'."}
    )
    cc_heads_types: List[str] = field(
        default=None,
        metadata={"help": f"A list of head types for the control concept. Currently supports only"
                          f" '{SEQUENCE_CLASSIFICATION}' and '{TOKEN_CLASSIFICATION}'."}
    )
...
def main():
    ...
    if data_args.task_name is not None:
        ...
        if causalm_training_args.causalm_additional_pretraining:
            raw_datasets = preprocess_cebab_for_causalm(raw_datasets, causalm_training_args.tc_labels_col, causalm_training_args.cc_labels_col)
    ...
    additional_pretraining_config = BertCausalmConfig(
        tc_heads_cfg=[CausalmHeadConfig(head_name=f'tc_head_{idx}',
                                        head_type=head_type,
                                        head_params={'num_labels': num_labels})
                      for idx, (head_type, num_labels) in enumerate(zip(model_args.tc_heads_types, model_args.tc_heads_num_labels))
                      ],
        cc_heads_cfg=[CausalmHeadConfig(head_name=f'cc_head_{idx}',
                                        head_type=head_type,
                                        head_params={'num_labels': num_labels},
    ...
    task_config = BertCausalmConfig(
        model_args.config_name if model_args.config_name else model_args.model_name_or_path,
        sequence_classifier_type='task',
        num_labels=num_labels,
        tc_lambda=model_args.tc_lambda,  # Just for fun, it doesn't do anything
    ...
    additional_pretraining_model = BertForCausalmAdditionalPreTraining(additional_pretraining_config)
    task_model = BertCausalmForNonlinearSequenceClassification(task_config)
    ...
    label_to_id = None
    if (
            task_model.config.label2id != PretrainedConfig(num_labels=num_labels).label2id
    ...
    if causalm_training_args.do_eval or causalm_training_args.causalm_additional_pretraining:
        ...
        eval_f_dataset = raw_datasets["validation"]
        if data_args.max_eval_samples is not None:
            eval_f_dataset = eval_f_dataset.select(range(data_args.max_eval_samples))
    ...
    if causalm_training_args.do_train or causalm_training_args.causalm_additional_pretraining:
        ...
        if data_args.task_name in task_to_keys:  # TODO
            accuracy_metric = load_metric('accuracy')

            def mean_cls_proba_metric(preds_probas, cls=1):
                return np.mean(preds_probas, axis=0)[cls]
            ...
            def compute_metrics(p: EvalPrediction):
                ...
                preds_probas = torch.softmax(torch.tensor(logits), dim=1).numpy()
                clf_report = classification_report(p.label_ids, preds, digits=5, output_dict=True)
                if data_args.task_name == 'causalm_treate':
                    result = accuracy_metric.compute(predictions=preds, references=p.label_ids)
                    result[f"mean_cls_{data_args.cls}_proba"] = mean_cls_proba_metric(preds_probas, cls=data_args.cls)
    ...
    if causalm_training_args.causalm_additional_pretraining:
        ...
        additional_pretraining_trainer = CausalmTrainer(
            ...
            eval_dataset=train_dataset if not eval_f_dataset else eval_f_dataset,
        ...
        # assign the adversarially trained model to be the underlying task model
        task_model.bert = additional_pretraining_trainer.model.bert  # todo generalize to other models

    task_trainer = Trainer(
        model=task_model,
        args=causalm_training_args,
        train_dataset=train_dataset if causalm_training_args.do_train else None,
        eval_dataset=eval_f_dataset if causalm_training_args.do_eval else None,
    ...
    if causalm_training_args.do_eval:
        ...
        eval_datasets = {'eval_f': eval_f_dataset}
        if data_args.task_name == "causalm_treate":
            for dataset_name, dataset in raw_datasets.items():
                if len(dataset_name.split('_')) == 1:
                    continue
                if dataset_name.split('_')[-1] == 'cf':
                    continue
                new_dataset_name = "_".join(dataset_name.split('_')[1:-1])
                eval_datasets[f'eval_{new_dataset_name}'] = dataset

        for dataset_name, eval_dataset in eval_datasets.items():
            metric_key_prefix = dataset_name + '_causalm'

            metrics = task_trainer.evaluate(eval_dataset=eval_dataset, metric_key_prefix=metric_key_prefix)
